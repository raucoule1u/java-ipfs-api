...
public class IPFS {
  ...
  public List<String> ObjectPatchTypes = Arrays
      .asList("add-link", "rm-link", "set-data", "append-data");
  ...
  public List<MerkleNode> add(List<NamedStreamable> files) throws IOException {
    Multipart m = new Multipart(
        "http://" + host + ":" + port + version + "add?stream-channels=true", "UTF-8");
  ...
  public List<MerkleNode> ls(Multihash hash) throws IOException {
    ...
    return ((List<Object>) res.get("Objects")).stream().map(x -> MerkleNode.fromJSON((Map) x))
        .collect(Collectors.toList());
  }
  ...
  public Map mount(java.io.File ipfsRoot, java.io.File ipnsRoot) throws IOException {
    ...
    return (Map) retrieveAndParse(
        "mount?arg=" + (ipfsRoot != null ? ipfsRoot.getPath() : "/ipfs") + "&arg=" +
  ...
  public class Pin {
    ...
    public List<Multihash> add(Multihash hash) throws IOException {
      return ((List<Object>) ((Map) retrieveAndParse("pin/add?stream-channels=true&arg=" + hash))
          .get("Pins"))
          .stream()
    ...
    public Map<Multihash, Object> ls(PinType type) throws IOException {
      return ((Map<String, Object>) (((Map) retrieveAndParse(
          "pin/ls?stream-channels=true&t=" + type.name())).get("Keys"))).entrySet()
          .stream()
    ...
    public List<MultiAddress> update(Multihash existing, Multihash modified, boolean unpin)
        throws IOException {
      return ((List<Object>) ((Map) retrieveAndParse(
          "pin/update?stream-channels=true&arg=" + existing + "&arg=" + modified + "&unpin="
              + unpin)).get("Pins"))
          .stream()
  ...
  public class Block {
    ...
    public MerkleNode put(byte[] data, Optional<String> format) {
      ...
      Multipart m = new Multipart(
          "http://" + host + ":" + port + version + "block/put?stream-channels=true" + fmt,
          "UTF-8");
      m.addFilePart("file", new NamedStreamable.ByteArrayWrapper(data));
      String res = m.finish();
      return JSONParser.parseStream(res).stream()
          .map(x -> MerkleNode.fromJSON((Map<String, Object>) x)).findFirst().get();
    }
  ...
  public class IPFSObject {
    ...
    public List<MerkleNode> put(List<byte[]> data) throws IOException {
      Multipart m = new Multipart(
          "http://" + host + ":" + port + version + "object/put?stream-channels=true", "UTF-8");
      for (byte[] f : data) {
        m.addFilePart("file", new NamedStreamable.ByteArrayWrapper(f));
      }
      String res = m.finish();
      return JSONParser.parseStream(res).stream()
          .map(x -> MerkleNode.fromJSON((Map<String, Object>) x)).collect(Collectors.toList());
    }
    ...
    public List<MerkleNode> put(String encoding, List<byte[]> data) throws IOException {
      ...
      Multipart m = new Multipart(
          "http://" + host + ":" + port + version + "object/put?stream-channels=true&encoding="
              + encoding, "UTF-8");
      for (byte[] f : data) {
        m.addFilePart("file", new NamedStreamable.ByteArrayWrapper(f));
      }
    ...
    public MerkleNode patch(Multihash base, String command, Optional<byte[]> data,
        Optional<String> name, Optional<Multihash> target) throws IOException {
      ...
      switch (command) {
        ...
        case "append-data":
          ...
          Multipart m = new Multipart(
              "http://" + host + ":" + port + version + "object/patch/" + command + "?arg=" + base
                  .toBase58() + "&stream-channels=true", "UTF-8");
  ...
  public class Name {
    ...
    public Map publish(Multihash hash, Optional<String> key) throws IOException {
      return retrieveMap("name/publish?arg=/ipfs/" + hash + (key.map(s -> "&key=" + s).orElse("")));
    }

    @Deprecated
    public Map publish(Optional<String> id, Multihash hash) throws IOException {
      return retrieveMap(
          "name/publish?arg=" + (id.map(s -> s + "&arg=").orElse("")) + "/ipfs/" + hash);
    }
  ...
  public List<MultiAddress> bootstrap() throws IOException {
    return ((List<String>) retrieveMap("bootstrap/").get("Peers")).stream()
  ...
  public class Bootstrap {
    ...
    public List<MultiAddress> rm(MultiAddress addr, boolean all) throws IOException {
      return ((List<String>) retrieveMap("bootstrap/rm?" + (all ? "all=true&" : "") + "arg=" + addr)
          .get("Peers")).stream().map(x -> new MultiAddress(x)).collect(Collectors.toList());
    }
  }
  ...
  public class Swarm {
    ...
    public List<Peer> peers() throws IOException {
      ...
      return ((List<Object>) m.get("Peers")).stream().map(Peer::fromJSON)
          .collect(Collectors.toList());
    }
  ...
  public class Dag {
    ...
    public MerkleNode put(String inputFormat, byte[] object, String outputFormat)
      ...
      Multipart m = new Multipart(
          prefix + "block/put/?stream-channels=true&input-enc=" + inputFormat + "&f="
              + outputFormat, "UTF-8");
  ...
  public class Config {
    ...
    public void replace(NamedStreamable file) throws IOException {
      Multipart m = new Multipart(
          "http://" + host + ":" + port + version + "config/replace?stream-channels=true", "UTF-8");
      m.addFilePart("file", file);
      String res = m.finish();
    }
  ...
  private static byte[] post(URL target, byte[] body, Map<String, String> headers)
      throws IOException {
    HttpURLConnection conn = (HttpURLConnection) target.openConnection();
